...
Parser* ConfigParseToConsole::Build()
{
  try
  {
	...
	// Namespace, class, struct and enum
	...
	//Try and Do
	pTryDo = new TryDo;
	pPushTryDo = new PushTryDo(pRepo);
	pParser->addRule(pTryDo);
	pTryDo->addAction(pPushTryDo);

	//Special Block
	pSplBlock = new SplBlock;
	pPushSplBlock = new PushSplBlock(pRepo);
	pParser->addRule(pSplBlock);
	pSplBlock->addAction(pPushSplBlock);
    return pParser;
  }
